...
assert(openaiEntries.length === 10, 'OpenAI deprecation fixture parses all entries')
assert(openaiEntries.find(entry => entry.id === 'o3-deep-research-2025-06-26')?.announced === '2026-04-22', 'OpenAI announced date parses')
assert(openaiEntries.find(entry => entry.id === 'o3-deep-research-2025-06-26')?.shutdown === '2026-07-23', 'OpenAI shutdown date parses')
...
assert(openaiCheck.code === 0, '--check exits 0 for the unchanged OpenAI fixture')
assert(openaiCheck.out.includes('No semantic changes.'), '--check prints the no-change result')
