...
export const lifecycleFor = (entry, options) => {
  ...
  if (sd) {
    const distribution = via
      ...
      : null
    const publisherClock = sd.via === 'publisher' || sd.via === 'publisher-fallback'
    const announced = publisherClock ? entry.announced : distribution?.announced
    ...
    if (sd.date && sd.date_precision === 'tentative' && !announced) {
      const policyFloor = publisherClock ? policySafeUntil(policy, generated) : null
...
export const captureWindowFor = (entry, lifecycle, { today = new Date() } = {}) => {
  if (!['retiring', 'retired', 'scheduled'].includes(lifecycle.status)) return null
  const todayDate = today.toISOString().slice(0, 10)
  const futureDate = shutdown => typeof shutdown === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(shutdown) && shutdown > todayDate
  const publisherClock = lifecycle.via === 'publisher' || lifecycle.via === 'publisher-fallback'
  const alternatives = (entry.distributions ?? [])
    .filter(row => (publisherClock || row.via !== lifecycle.via) && row.status !== 'retired' && futureDate(row.shutdown))
    .map(row => ({ via: row.via, until: row.shutdown, date_precision: row.date_precision ?? null, status: row.status ?? null }))
  if (!publisherClock && futureDate(entry.shutdown)) {
    alternatives.push({ via: 'publisher', until: entry.shutdown, date_precision: entry.date_precision ?? null, status: null })
  }
  const compare = (a, b) => a < b ? -1 : a > b ? 1 : 0
  alternatives.sort((a, b) => compare(a.until, b.until) || compare(a.via, b.via))
  const until = lifecycle.status === 'retired' ? null : lifecycle.shutdown
  return {
    until,
    date_precision: until !== null ? lifecycle.date_precision ?? null : null,
    via: lifecycle.via,
    alternatives,
  }
}
...
export const findingFromRef = (ref, options) => {
  ...
  const finding = {
    ...
    threshold_days: days,
    capture: captureWindowFor(ref.entry, lifecycle, { today }),
    ...(waiver ? { waiver } : {}),
  }
  return finding
}
