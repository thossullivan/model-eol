...
export const lifecycleFor = (entry, options) => {
  ...
  if (sd) {
    ...
    const tentativeFloor = publisherClock && sd.date_precision === 'earliest' && !entry.announced
    if (sd.date && tentativeFloor) {
      ...
      Object.defineProperty(lifecycle, '_tentative_floor', { value: true })
      return lifecycle
    }
...
export const findingFromRef = (ref, options) => {
  ...
  if (lifecycle._tentative_floor) Object.defineProperty(finding, '_tentative_floor', { value: true })
  return finding
}
