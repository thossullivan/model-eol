...
export const validateFeed = feed => {
  ...
  for (const [index, model] of feed.models.entries()) {
    ...
    if (model.date_precision === 'tentative' && model.shutdown === undefined) {
      fail(`${at}.shutdown`, 'is required when date_precision is tentative')
    }
    ...
    if (model.distributions !== undefined) {
      if (!Array.isArray(model.distributions)) {
        ...
        for (const [distributionIndex, distribution] of model.distributions.entries()) {
          ...
          if (distribution.date_precision === 'tentative' && distribution.shutdown === undefined) {
            fail(`${dAt}.shutdown`, 'is required when date_precision is tentative')
          }
