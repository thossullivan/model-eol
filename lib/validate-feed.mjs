...
const DATE_PRECISIONS = new Set(['exact', 'earliest'])
...
export const validateFeed = feed => {
  ...
  for (const [index, model] of feed.models.entries()) {
    ...
    if (model.date_precision !== undefined) {
      if (typeof model.date_precision !== 'string' || !DATE_PRECISIONS.has(model.date_precision)) fail(`${at}.date_precision`, 'must be exact or earliest')
    }
    ...
    if (model.distributions !== undefined) {
      if (!Array.isArray(model.distributions)) {
        ...
        for (const [distributionIndex, distribution] of model.distributions.entries()) {
          ...
          if (distribution.date_precision !== undefined && (typeof distribution.date_precision !== 'string' || !DATE_PRECISIONS.has(distribution.date_precision))) fail(`${dAt}.date_precision`, 'must be exact or earliest')
          ...
          if (distribution.shutdown && !(distribution.source || model.source || feed.source)) fail(dAt, 'dated distribution needs a source')
        }
      }
    }
  }
