...
import { assertIsoDate, MAX_REPORT_BYTES, MAX_TIMEOUT_MS, MIN_TIMEOUT_MS } from './validate-feed.mjs'
...
const PUBLISHER_WAIVER_VIAS = new Set(['publisher', 'publisher-fallback'])
...
const copyDefaults = defaults => ({
  ...
  waivers: (defaults.waivers ?? []).map(waiver => ({
    ...waiver,
    ...(waiver.paths ? { paths: [...waiver.paths] } : {}),
  })),
  overrides: (defaults.overrides ?? []).map(override => ({
    ...
    ...(override.waivers ? { waivers: override.waivers.map(waiver => ({
      ...waiver,
      ...(waiver.paths ? { paths: [...waiver.paths] } : {}),
    })) } : {}),
  })),
...
const boundedString = (value, label, limit) => {
  nonEmptyString(value, label)
  assert(value.length <= limit, `${label} must be at most ${limit} characters`)
  return value
}
...
const normalizeWaiver = (raw, index, labelPrefix) => {
  const label = `${labelPrefix}[${index}]`
  assert(raw && typeof raw === 'object' && !Array.isArray(raw), `${label} must be an object`)
  assertKnownKeys(raw, label, new Set(['model', 'paths', 'via', 'reason', 'owner', 'expires']))
  const waiver = {
    model: nonEmptyString(raw.model, `${label}.model`),
    reason: boundedString(raw.reason, `${label}.reason`, 500),
    owner: boundedString(raw.owner, `${label}.owner`, 200),
    expires: nonEmptyString(raw.expires, `${label}.expires`),
  }
  try {
    assertIsoDate(waiver.expires, `${label}.expires`)
  } catch (error) {
    assert(false, error.message)
  }
  if (raw.paths !== undefined) waiver.paths = stringArray(raw.paths, `${label}.paths`)
  if (raw.via !== undefined) waiver.via = nonEmptyString(raw.via, `${label}.via`)
  return waiver
}

const normalizeWaivers = (raw, label) => {
  assert(Array.isArray(raw), `${label} must be an array`)
  assert(raw.length <= MAX_WAIVERS, `${label} must contain at most ${MAX_WAIVERS} waivers`)
  return raw.map((waiver, index) => normalizeWaiver(waiver, index, label))
}

const normalizeOverride = (raw, index) => {
  const label = `overrides[${index}]`
  assert(raw && typeof raw === 'object' && !Array.isArray(raw), `${label} must be an object`)
  assertKnownKeys(raw, label, new Set(['paths', 'days', 'scope', 'via', 'ignore', 'waivers']))
  ...
  if (raw.waivers !== undefined) override.waivers = normalizeWaivers(raw.waivers, `${label}.waivers`)
  return override
}
...
export const normalizeConfig = (raw, { defaults = DEFAULT_CONFIG } = {}) => {
  ...
  assertKnownKeys(raw, '', new Set(['days', 'scope', 'via', 'feeds', 'ignore', 'waivers', 'overrides', 'routes', 'issues', 'eval']))
  ...
  if (raw.waivers !== undefined) config.waivers = normalizeWaivers(raw.waivers, 'waivers')
  if (raw.overrides !== undefined) {
    assert(Array.isArray(raw.overrides), 'overrides must be an array')
    config.overrides = raw.overrides.map(normalizeOverride)
  }
  const waiverCount = config.waivers.length + config.overrides.reduce((count, override) => count + (override.waivers?.length ?? 0), 0)
  assert(waiverCount <= MAX_WAIVERS, `config must contain at most ${MAX_WAIVERS} waivers across the root and overrides`)
...
export const configForRepoPath = (config, repoPath) => {
  ...
  const matchesPath = patterns => matchesAnyGlob(normalizedPath, patterns) || patterns.some(pattern => {
    const normalizedPattern = normalizeRepoPath(pattern).replace(/\/+$/, '')
    return normalizedPattern && !/[*?]/.test(normalizedPattern) && normalizedPath.startsWith(`${normalizedPattern}/`)
  })
  const applicableWaivers = waivers => (waivers ?? []).filter(waiver => waiver.paths === undefined || matchesPath(waiver.paths))
  const policy = {
    days: config.days,
    scope: config.scope,
    via: config.via,
    ...
    waivers: applicableWaivers(config.waivers),
    override_indexes: [],
  }
  for (const [index, override] of (config.overrides ?? []).entries()) {
    ...
    policy.waivers.push(...applicableWaivers(override.waivers))
  }
...
export const configuredVias = config => [
  ...
  ...(config.waivers ?? []).map(waiver => waiver.via).filter(via => !PUBLISHER_WAIVER_VIAS.has(via)),
  ...(config.overrides ?? []).flatMap(override => (override.waivers ?? []).map(waiver => waiver.via).filter(via => !PUBLISHER_WAIVER_VIAS.has(via))),
