...
const collectFilesDetailed = (targets, { includeDocs = false } = {}) => {
  const files = []
  const notes = []
  const validTargets = []
  const seen = new Set()
  ...
  const addFile = absolute => {
    if (stopped || seen.has(absolute)) return
    seen.add(absolute)
    ...
    if (files.length >= MAX_FILES) {
      stopped = true
      if (!fileLimitWarned) {
        console.error(`model-eol: warning: file-count cap (${MAX_FILES}) reached; remaining files skipped`)
  ...
  const listGitFiles = (absoluteTarget, gitRoot) => {
    ...
    if (result.error || result.status !== 0) {
      console.error(`model-eol: warning: git file listing failed for ${displayPath(absoluteTarget)}; no recursive fallback used`)
      return
    }
...
export const scanTargets = ({ targets, entries, keys, includeDocs = false }) => {
  const collected = collectFilesDetailed(targets, { includeDocs })
  ...
  for (const file of files) {
    ...
    // A model-eol feed document is lifecycle data, not model usage - scanning
    // one would flag every retired id it exists to describe.
