...
export const findingTail = (f, days) =>
  f.status === 'retired' ? `RETIRED ${shutdownText(f)} (${-f.days} days ago)${f.replacement ? ` -> ${f.replacement}` : ''}` :
  f.status === 'retiring' ? `RETIRES ${shutdownText(f)} (${f.days} days)${f.replacement ? ` -> ${f.replacement}` : ''}` :
  f.status === 'scheduled' ? `scheduled ${shutdownText(f)} (${f.days} days, outside --days ${days})` :
...
export const formatInventory = (inventory, days) => {
  ...
  if ((inventory.scan_notes ?? []).length) out.push(`WARNING: scan incomplete; ${inventory.scan_notes.length} scan note(s) recorded`)
...
export const formatSchedule = (schedule, days) => {
  ...
  if ((schedule.scan_notes ?? []).length) out.push(`WARNING: scan incomplete; ${schedule.scan_notes.length} scan note(s) recorded`)
...
export const formatInventoryCycloneDX = inventory => {
  ...
  return {
    ...
    specVersion: '1.6',
    version: 1,
    metadata: { timestamp: inventory.generated },
...
export const formatAlertGithub = (alert, days) => {
  const out = []
  if ((alert.scan_notes ?? []).length) out.push(`::warning title=${annotationEscape('model-eol scan')}::${annotationEscape(`Scan incomplete; ${alert.scan_notes.length} scan note(s) recorded`)}`)
...
export const formatAlertMarkdown = (alert, days) => {
  ...
  if ((alert.scan_notes ?? []).length) out.push(`> Warning: scan incomplete; ${alert.scan_notes.length} scan note(s) recorded.`, '')
