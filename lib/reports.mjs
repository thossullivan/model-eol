...
export const formatInventoryCycloneDX = inventory => {
  ...
  for (const item of [...inventory.model_references].sort(compareReferences)) {
    ...
    if (!group.activeWaiver && item.waiver?.active === true) group.activeWaiver = item.waiver
    group.usages.add(item.usage)
    group.occurrences.add(`${item.file}#${item.line}`)
  }
      ...
      properties: [
        ...
        ...(group.activeWaiver
          ? [
              ...
              { name: 'model-eol:waiver_owner', value: group.activeWaiver.owner },
              { name: 'model-eol:waiver_expires', value: group.activeWaiver.expires },
              { name: 'model-eol:waiver_reason', value: group.activeWaiver.reason },
            ]
          : []),
