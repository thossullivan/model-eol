...
const shutdownText = finding =>
  finding.date_precision === 'tentative' ? `not sooner than ${finding.shutdown}` :
  finding.date_precision === 'earliest' ? `no earlier than ${finding.shutdown}` :
  finding.shutdown

const tentativeFloor = finding => finding.date_precision === 'tentative'

const policyTail = finding => finding.safe_until && finding.shutdown && finding.safe_until > finding.shutdown
  ? `, guaranteed until ${finding.safe_until} by ${finding.publisher} policy`
  : ''
...
const lifecycleTail = (f, days) =>
  ...
  tentativeFloor(f) ? `scheduled - tentative, not announced: ${shutdownText(f)}${policyTail(f)}` :
...
const captureTail = finding => {
  if (!['retiring', 'retired'].includes(finding.status) || !finding.capture?.alternatives?.length) return ''
  const alternatives = finding.capture.alternatives.map(alternative => {
    const until = ['tentative', 'earliest'].includes(alternative.date_precision) ? 'at least until' : 'until'
    return `via ${alternative.via} ${until} ${alternative.until}`
  })
  return `  [still answers ${alternatives.join(', ')}]`
}

export const findingTail = (finding, days) => `${lifecycleTail(finding, days)}${waiverTail(finding)}${captureTail(finding)}`
...
export const formatInventoryCycloneDX = inventory => {
  ...
  for (const item of [...inventory.model_references].sort(compareReferences)) {
    ...
    const occurrenceWaiver = group.occurrenceWaivers.get(location) ?? { active: true, activeWaivers: [], expiredWaivers: [] }
    if (item.waiver?.active === true) occurrenceWaiver.activeWaivers.push(item.waiver)
    else {
      occurrenceWaiver.active = false
      if (item.waiver?.active === false) occurrenceWaiver.expiredWaivers.push(item.waiver)
    }
  ...
  const components = [...groups.values()].map(group => {
    ...
    const activeWaivers = occurrenceWaivers.flatMap(occurrence => occurrence.activeWaivers)
    const expiredWaivers = occurrenceWaivers.flatMap(occurrence => occurrence.expiredWaivers)
    const everyOccurrenceWaived = occurrenceWaivers.every(occurrence => occurrence.active && occurrence.activeWaivers.length > 0)
    ...
    } else if (expiredWaivers.length > 0) {
      const latestExpiry = expiredWaivers.map(waiver => waiver.expires).sort().at(-1)
      const distinctWaivers = new Set(expiredWaivers.map(waiver => JSON.stringify([waiver.owner, waiver.expires, waiver.reason])))
      waiverProperties = [
        { name: 'model-eol:waiver_active', value: 'false' },
        ...(distinctWaivers.size === 1
          ? [
              { name: 'model-eol:waiver_owner', value: expiredWaivers[0].owner },
              { name: 'model-eol:waiver_expires', value: latestExpiry },
              { name: 'model-eol:waiver_reason', value: expiredWaivers[0].reason },
            ]
          : [{ name: 'model-eol:waiver_expires', value: latestExpiry }]),
      ]
    }
