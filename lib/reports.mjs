...
export const formatInventory = (inventory, days) => {
  const out = [`model-eol inventory: ${inventory.summary.model_references} tracked model reference(s), ${inventory.summary.candidate_model_references} candidate(s), ${inventory.summary.integration_hints} integration hint(s) across ${inventory.scanned_files} files`]
...
export const formatSchedule = (schedule, days) => {
  const out = [`model-eol schedule: ${schedule.items.length} scheduled/watch finding(s), ${schedule.candidate_model_references.length} candidate(s), ${schedule.unresolved_integrations.length} unresolved cloud/gateway hint(s)`]
  for (const item of schedule.items) {
    const safeUntil = item.safe_until ? `  [safe until ${item.safe_until}]` : ''
    out.push(`${marker(item.status)} ${item.shutdown ?? 'TBD'}  ${item.file}:${item.line}  ${item.matched}  ${findingTail(item, days)}  [${item.usage}]${safeUntil}`)
  }
  ...
  out.push(schedule.earliest_risk
